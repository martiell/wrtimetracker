...
// +----------------------------------------------------------------------+
	...
	$cl_id = $request->getAttribute("client_id");
	if (!is_null($cl_id)) {
		$_SESSION["cl_client_id"] = $cl_id;
	}
	if (is_null($cl_id)) {
		$cl_id = @$_SESSION["cl_client_id"];
	}
	...
	if ($request->getMethod()=="POST") {
		...
		if (!is_null($cl_id)) {
			...
			$cl_name  = $ud['clnt_name'];
			$cl_yourcoo = $ud['clnt_addr_your'];
			$cl_custcoo = $ud['clnt_addr_cust'];
			$cl_comment = $ud['clnt_comment'];
			$cl_tax = $ud['clnt_tax'];
			$cl_daily_subtotals = $ud['clnt_fsubtotals'];
			$cl_discount = $ud['clnt_discount'];
		}
	}
	...
	if ($request->getMethod()=="POST") {
        ...
        if ($errors->isEmpty()) {
        	ClientHelper::update($user, array(
              'id' => $cl_id,
              'name' => $cl_name,
              'address' => $cl_custcoo,
              'tax' => $cl_tax,
