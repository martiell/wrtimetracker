    ...
    function initAttributes(&$request) {
        ...
        if ($submit_flag) {
	    	...
	    	foreach ($this->mVariables as $name) {
	    		if (is_array($request->getParameter($name))) {
	    			if ($this->mForm->mElements[$name]) {
	    			...
	    			if ($this->mForm->mElements[$name]) {
					    $this->mForm->mElements[$name]->setValue(stripslashes($request->getParameter($name)));
        ...
        if ($this->mForm) {
        	if (isset($this->mForm->mElements[$name])) {
        		if ($this->mForm->mElements[$name]->cClassName=="DateField") {
        			$dt = new DateAndTime($GLOBALS["I18N"]->getDateFormat(), $value);
					$value = $dt->toString(SYS_DATEFORMAT);
        		}
