...
class CheckboxGroup extends FormElement {
	...
	function setLocalization($i18n) {
		...
		if ($i18n->getKey('label.sel_all'))
			$this->lSelAll		= $i18n->getKey('label.sel_all');
		if ($i18n->getKey('label.sel_none'))
			$this->lSelNone		= $i18n->getKey('label.sel_none');
		if ($i18n->getKey('label.sel_project'))
			$this->lSelProject	= $i18n->getKey('label.sel_projects');
	}
	    	...
	    	if (is_array($this->mOptions)) {
			    ...
			    foreach ($this->mOptions as $optkey=>$optval) {
			    	...
			    	if (is_array($this->mValue)) {
				   	...
				   	$html .= " value=\"".htmlspecialchars($optkey)."\">&nbsp;".htmlspecialchars($optval);
