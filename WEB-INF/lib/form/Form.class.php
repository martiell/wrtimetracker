	...
	function addInput($arguments) {
		...
		switch($arguments["type"]) {
			...
			case "calendar":
			    ...
			    if (isset($arguments["dateformat"])) $el->setDateFormat($arguments["dateformat"]);
			    if (isset($arguments["sysdateformat"])) $el->setSysDateFormat($arguments["sysdateformat"]);
			    break;  
...
/*    function proccessValues() {
    	foreach ($this->mElements as $elname=>$el) {
            if ($this->mFormBean->getAttribute($elname)!="" && $this->mElements[$elname]) {
			    $this->mElements[$elname]->setValue(stripslashes($this->mFormBean->getAttribute($elname)));
    	    }
        }
    }
*/    
