...
// +----------------------------------------------------------------------+
  ...
  if (MULTITEAM_MODE!="true" || $auth->isPasswordExternal()) {
  ...
  if ($request->getMethod()=="POST") {
    ...
    if (!UserHelper::findUserByLogin($cl_login)) {
      if (UserHelper::insertAccount(array(
          ...
          'email' => $cl_email))) {
        // authorizeUserHelper::insertAccount($cl_name, $cl_login, $cl_password1, $cl_company, $cl_www, $cl_currency, true, null, $cl_email)
        if ($auth->doLogin($cl_login, $cl_password1)) {
